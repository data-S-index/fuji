...
def evaluate_fairness(
    ...
    metadata_service_endpoint: Optional[str] = None,
    oaipmh_endpoint: Optional[str] = None,
    metadata_service_type: Optional[str] = None,
    use_datacite: bool = True,
    use_github: bool = False,
    metric_version: Optional[str] = None,
    auth_token: Optional[str] = None,
    auth_token_type: Optional[str] = None,
    allow_remote_logging: bool = False,
) -> FAIRResults:
    """
    Evaluate FAIRness of a data object based on its identifier.

    This is the core assessment function that can be called directly from Python
    without requiring HTTP requests. The HTTP endpoint /evaluate wraps this function.

    Args:
        object_identifier: The identifier (e.g., DOI, URL) of the object to assess
        test_debug: Enable debug mode for detailed logging
        metadata_service_endpoint: URL of metadata service endpoint
        oaipmh_endpoint: OAI-PMH endpoint URL
        metadata_service_type: Type of metadata service (e.g., 'oai_pmh')
        use_datacite: Whether to use DataCite for metadata harvesting
        use_github: Whether to use GitHub API for metadata harvesting
        metric_version: Version of metrics to use (e.g., 'metrics_v0.8')
        auth_token: Authentication token for accessing protected resources
        auth_token_type: Type of authentication token ('Basic' or 'Bearer')
        allow_remote_logging: Whether to enable remote logging
    ...
    logger.info("Assessment target: " + object_identifier)
    print("Assessment target: ", object_identifier, flush=True)
    starttimestmp = datetime.datetime.now().replace(microsecond=0).isoformat() + "Z"
    ft = FAIRCheck(
        uid=object_identifier,
    ...
    remote_log_host = Preprocessor.remote_log_host
    remote_log_path = Preprocessor.remote_log_path
    ...
    metadata_identifier_included_result = (
        ft.check_metadata_identifier_included_in_metadata()
    )
...
async def assess_by_id(body):
    ...
    This is the HTTP endpoint wrapper that calls evaluate_fairness() internally.
    ...
    if connexion.request.content_type == "application/json":
        # The client has to send this HTTP header (Allow-Remote-Logging:True)
        # explicitly to enable remote logging. Useful for e.g. web clients.
        allow_remote_logging = connexion.request.headers.get("Allow-Remote-Logging")

        # Extract parameters from request body
        identifier = body.get("object_identifier")
        debug = body.get("test_debug", True)
        ...
        usedatacite = body.get("use_datacite", True)
        usegithub = body.get("use_github", False)
        metric_version = body.get("metric_version")
        print("BODY METRIC", metric_version)
        auth_token = body.get("auth_token")
        auth_token_type = body.get("auth_token_type")

        # Call the core assessment function
        final_response = evaluate_fairness(
            ...
            allow_remote_logging=bool(allow_remote_logging),
        )

        # Handle HTTP response formatting based on Accept header
        ...
        accept_header_value = connexion.request.headers.get("Accept")
        if accept_header_value in rdf_mimes:
            rdf_mapper = FAIRResultsMapper(final_response)
            rdf = rdf_mapper.getQualityVocabularyRDF(accept_header_value)
            print("RDF")
            return rdf, 200, {"content-type": accept_header_value}
        # Standard JSON
